...
const fs = require('fs');
const pluginName = 'Custom-Plugin-Digicopy-Online';
...
fs.rmSync(`${destination}/app`, { recursive: true, force: true });
...
/*mv('./dist', destination, {mkdirp: true}, function(err) {
    // done. it first created all the necessary directories, and then
    // tried fs.rename, then falls back to using ncp to copy the dir
    // to dest and then rimraf to remove the source dir
  });*/
execSync(`Xcopy "./dist" "${destination}" /E /H /C /I`)

// copy the js and css file names to an array
distFilenames = fs.readdirSync(`${destination}/app`);
